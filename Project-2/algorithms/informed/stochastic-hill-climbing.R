stochastic.hill.climbing = function(problem,
  ...
  name_method      <- paste0("Stochastic Hill Climbing")
  ...
  while (count <= max_iterations) {
    ...
    # Filter successor nodes with evaluation better than current node
    filtered_nodes <- list()
    
    for (node in successor_nodes) {
      # print(node$evaluation)
      # print(node_current$evaluation)
      # print("---")
      if (node$evaluation <= node_current$evaluation){
        filtered_nodes <- append(filtered_nodes, list(node))
      }
    }
