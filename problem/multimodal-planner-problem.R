...
initialize.problem <- function(file) {
  ...
  # Load CSV file
  data <- load.from.csv(file) # V1=y=row, V2=x=col
  ...
  problem$map <- matrix(list(), nrow = data$V1[[1]], ncol = data$V2[[1]]) # Matriz de listas de transportes (caminar es el default /lista vacia/)
  problem$time_cost_list <- list(w=data$time_cost[[4]], e=data$time_cost[[5]]) # costes de tiempo cada modo de transporte
  problem$cost_list <- list(w=data$cost[[4]], e=data$cost[[5]]) # costes de cada modo de transporte
